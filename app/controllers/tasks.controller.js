...
exports.createNewTask = async (req, res) => {
  ...
  const data = {
    ...
    reward_type: body.hasOwnProperty("Reward Type")
      ? req.body["Reward Type"]
      : 0,
    reward_center_id: body.hasOwnProperty("Reward Center Id")
      ? req.body["Reward Center Id"]
      : 0,
    audience: body.hasOwnProperty("Audience") ? req.body["Audience"] : 0,
    bonus_reward_type: body.hasOwnProperty("Bonus Reward Type")
      ? req.body["Bonus Reward Type"]
      : 0,
    bonus_set_id: body.hasOwnProperty("Bonus Set Id")
      ? req.body["Bonus Set Id"]
      : 0,
    bonus_item_id: body.hasOwnProperty("Bonus Item Id")
      ? req.body["Bonus Item Id"]
      : 0,
    tickets_per_task_submissions: body.hasOwnProperty("Tickets Per Submissions")
      ? req.body["Tickets Per Submissions"]
      : 0,
    ta_media: body.hasOwnProperty("Task Media") ? req.body["Task Media"] : [],
    ta_desc: body.hasOwnProperty("Task description")
      ? req.body["Task description"]
      : "",
    ta_hashtag: body.hasOwnProperty("Task hashtags")
      ? req.body["Task hashtags"]
      : "",
    ta_token_budget: body.hasOwnProperty("Task token budget")
      ? req.body["Task token budget"]
      : 0,
    ta_budget_per_user: body.hasOwnProperty("Task budget per user")
      ? req.body["Task budget per user"]
      : 0,
    ta_stars_per_user: body.hasOwnProperty("Task stars per user")
      ? req.body["Task stars per user"]
      : 0,
    ta_energy_per_user: body.hasOwnProperty("Task energy per user")
      ? req.body["Task energy per user"]
      : 0,
    ta_total_available: body.hasOwnProperty("Task total available")
      ? req.body["Task total available"]
      : 0,
    ta_remaining_budget: body.hasOwnProperty("Task total available")
      ? req.body["Task total available"]
      : 0,
    ta_estimated_user: body.hasOwnProperty("Task estimated user")
      ? req.body["Task estimated user"]
      : 0,
    ta_header_image: body.hasOwnProperty("Task Header Image")
      ? req.body["Task Header Image"]
      : "",
    ta_do: body.hasOwnProperty("Task do") ? req.body["Task do"] : "",
    ta_dont_do: body.hasOwnProperty("Task dont do")
      ? req.body["Task dont do"]
      : "",
    ta_insta_question: body.hasOwnProperty("Task insta question")
      ? req.body["Task insta question"]
      : "",
    ta_photos_required: body.hasOwnProperty("Task photos required")
      ? req.body["Task photos required"]
      : 0,
    ta_videos_required: body.hasOwnProperty("Task videos required")
      ? req.body["Task videos required"]
      : 0,
    ta_mentioned: body.hasOwnProperty("Task mentioned")
      ? req.body["Task mentioned"]
      : "",
    ta_start_date: body.hasOwnProperty("Task start date")
      ? req.body["Task start date"]
      : new Date(),
    ta_end_date: body.hasOwnProperty("Task end date")
      ? req.body["Task end date"]
      : new Date(new Date().setFullYear(new Date().getFullYear() + 2)),
    ta_status: body.hasOwnProperty("Task status") ? req.body["Task status"] : 0,
    ta_oneline_summary: body.hasOwnProperty("Task oneline summary")
      ? req.body["Task oneline summary"]
      : "",
    ta_contiue_spend_budget: body.hasOwnProperty("Continue after budget spend")
      ? req.body["Continue after budget spend"]
      : false,
  };
  Tasks.create(data)
    .then((data) => {
    ...
    .catch((err) => {
      logger.log(
        "error",
        err + ": Some error occurred while creating the Tasks"
      );
      res.status(500).send({
...
exports.listing = async (req, res) => {
  ...
  const contentUserTaskIds = await common.getContentReportUser(
    ["Task", "Campaign", "User Task Post"],
    UserId
  );
  let taskIdsValues = [];
    ...
    include: [
      {
        model: db.campaigns,
        required: false,
        attributes: [["cp_campaign_name", "Campaign Name"]],
        where: { is_autotakedown: 0 },
      },
      {
        model: Posts,
        attributes: ["ucpl_u_id", "ucpl_content_id"],
        required: false,
        where: {
          ucpl_status: 1,
          is_autotakedown: 0,
  ...
  if (
    req.query.sortVal &&
    (req.query.sortBy == "ta_name" || req.query.sortBy == "ta_type")
  ) {
...
exports.taskListing = async (req, res) => {
  ...
  if (taskIds.length) {
      ...
      include: [
        {
          ...
          attributes: [
            ...
            ["cr_co_status", "brand_status"],
          ],
        },
        {
          model: db.user_content_post,
          attributes: [
            ["ucpl_content_id", "ucpl_content_id"],
            ["ucpl_id", "post_id"],
    ...
    var reward_given_options = {
      ...
      attributes: [
        ...
        [
          sequelize.fn("sum", sequelize.col("rewards_award_stars")),
          "rewards_award_stars",
        ],
        [
          sequelize.fn("sum", sequelize.col("rewards_award_token")),
          "rewards_award_token",
        ],
      ],
    ...
    var bonus_ticket_options = {
      ...
      attributes: [
        "event_id",
        [
          sequelize.fn("sum", sequelize.col("tickets_earned")),
          "tickets_earned",
        ],
      ],
      group: ["event_id"],
    };
    bonus_ticket_listing = await db.bonus_ticket_details.findAll(
      bonus_ticket_options
    );
  }
  ...
  if (contestIds.length) {
    ...
    contest_reward_given_listing = await db.rewards_given.findAll(
    ...
    contest_bonus_ticket_listing = await db.bonus_ticket_details.findAll(
      bonus_ticket_options
    );
  }
  if (surveyIds.length) {
  ...
  if (bonus_ticket_listing.length) {
    for (const bonus_ticket_key in bonus_ticket_listing) {
      ...
      bonusTickets[bonus_ticket_list.event_id + "_1"] =
        bonus_ticket_list.tickets_earned;
    }
  }
  if (contest_bonus_ticket_listing.length) {
    for (const bonus_ticket_key in contest_bonus_ticket_listing) {
      ...
      bonusTickets[bonus_ticket_list.event_id + "_2"] =
        bonus_ticket_list.tickets_earned;
    }
  }
  ...
  if (reward_given_listing.length) {
    for (const reward_given_key in reward_given_listing) {
      ...
      rewardTaskStars[reward_listing.rewards_award_event_id + "_1"] =
        reward_listing.rewards_award_stars;
      rewardTaskTokens[reward_listing.rewards_award_event_id + "_1"] =
  ...
  if (contest_reward_given_listing.length) {
    for (const reward_given_key in contest_reward_given_listing) {
      ...
      rewardTaskStars[reward_listing.rewards_award_event_id + "_2"] =
        reward_listing.rewards_award_stars;
      rewardTaskTokens[reward_listing.rewards_award_event_id + "_2"] =
        reward_listing.rewards_award_token;
    }
  }
  ...
  if (user_content_task_listing.length) {
    for (const user_content_key in user_content_task_listing) {
      ...
      userTaskPostData[
        content_task_list["ta_task_id"] +
          "_" +
          content_task_list["ucpl_content_type"]
      ] = content_task_list;
    }
  }
  ...
  if (userCompletedRewardData.length) {
    for (const userCompletedRewardDataKey in userCompletedRewardData) {
      const content_task_list =
        userCompletedRewardData[userCompletedRewardDataKey];
      userTaskPostCompleteData[
        content_task_list["ta_task_id"] +
          "_" +
          content_task_list["ucpl_content_type"]
      ] = content_task_list;
  ...
  if (tasks_list.length) {
    for (const tasks_list_key in tasks_list) {
      ...
      taskIdPostCount = 0;
      if (
        userTaskPostData[taskId + "_1"] &&
        userTaskPostData[taskId + "_1"].count
      ) {
        taskIdPostCount = userTaskPostData[taskId + "_1"].count;
      }
      taskIdPostCompletedCount = 0;
      if (
        userTaskPostCompleteData[taskId + "_1"] &&
        userTaskPostCompleteData[taskId + "_1"].count
      ) {
        taskIdPostCompletedCount =
          userTaskPostCompleteData[taskId + "_1"].count;
      }
      var brandPostCount = user_contentbrand_listing || 0;
      tasks_list[
        tasks_list_key
      ].dataValues.list_data.following_users_not_completed =
        brandPostCount - taskIdPostCount;
      tasks_list[
        tasks_list_key
      ].dataValues.list_data.entries_after_reward_completed = taskIdPostCompletedCount;
      tasks_list[tasks_list_key].dataValues.list_data.campaign_budget =
        tasks_list[tasks_list_key].ta_token_budget;
      tasks_list[tasks_list_key].dataValues.list_data.tickets =
        bonusTickets[taskId + "_1"] || 0;
      tasks_list[tasks_list_key].dataValues.list_data.stars_given =
        rewardTaskStars[taskId + "_1"] || 0;
      tasks_list[tasks_list_key].dataValues.list_data.tokens_given =
        rewardTaskTokens[taskId + "_1"] || 0;
      tasks_list[tasks_list_key].dataValues.list_data.brand_data =
        tasks_list[tasks_list_key].brand;
      const total_budget = tasks_list[tasks_list_key].ta_token_budget || 0;
      const tokens_remaining =
        tasks_list[tasks_list_key].ta_remaining_budget || 0;
      tasks_list[tasks_list_key].dataValues.list_data.budget_left =
        parseFloat(total_budget) - parseFloat(tokens_remaining);
      tasks_list[tasks_list_key].dataValues.list_data.task_entries = tasks_list[
        tasks_list_key
      ].user_content_posts
        ? tasks_list[tasks_list_key].user_content_posts.length
        : 0;
  ...
  if (contest_list.length) {
    for (const contest_list_key in contest_list) {
      ...
      contest_list[contest_list_key].dataValues.list_data.campaign_type =
        "Contest";

      taskIdPostCount = 0;
      if (
        userTaskPostData[taskId + "_2"] &&
        userTaskPostData[taskId + "_2"].count
      ) {
        taskIdPostCount = userTaskPostData[taskId + "_2"].count;
      }
      taskIdPostCompletedCount = 0;
      if (
        userTaskPostCompleteData[taskId + "_2"] &&
        userTaskPostCompleteData[taskId + "_2"].count
      ) {
        taskIdPostCompletedCount =
          userTaskPostCompleteData[taskId + "_2"].count;
      }
      ...
      contest_list[contest_list_key].dataValues.list_data.campaign_budget =
        contest_list[contest_list_key].ct_token_budget;
      contest_list[contest_list_key].dataValues.list_data.brand_data =
        contest_list[contest_list_key].brand;
      contest_list[contest_list_key].dataValues.list_data.tickets =
        bonusTickets[taskId + "_2"] || 0;
      contest_list[contest_list_key].dataValues.list_data.stars_given =
        rewardTaskStars[taskId + "_2"] || 0;
      contest_list[contest_list_key].dataValues.list_data.tokens_given =
        rewardTaskTokens[taskId + "_2"] || 0;
      const total_budget = contest_list[contest_list_key].ct_token_budget || 0;
      const tokens_remaining = rewardTaskTokens[taskId + "_2"] || 0;
      contest_list[contest_list_key].dataValues.list_data.budget_left =
        parseFloat(total_budget) - parseFloat(tokens_remaining);
      contest_list[contest_list_key].dataValues.list_data.task_entries =
        contest_list[contest_list_key].user_content_posts
          ? contest_list[contest_list_key].user_content_posts.length
          : 0;
  ...
  if (survey_list.length) {
    ...
    const SurveySubmissionsResult = await db.survey_submissions.count(
      SubmissionOptions
    );
    if (SurveySubmissionsResult.length) {
      for (const SurveySubmissionsKey in SurveySubmissionsResult) {
        surveySubmissionList[
          SurveySubmissionsResult[SurveySubmissionsKey].srs_sr_id
        ] = SurveySubmissionsResult[SurveySubmissionsKey];
      }
    }
    ...
    if (surveyBrandSubmissions.length) {
      for (const surveyBrandKey in surveyBrandSubmissions) {
        surveyBrandSubmissionList[
          surveyBrandSubmissions[surveyBrandKey].brand_id
        ] = surveyBrandSubmissions[surveyBrandKey];
      }
    }
    ...
    for (const survey_list_key in survey_list) {
      ...
      survey_list[survey_list_key].dataValues.list_data.campaign_type =
        "Survey";
      ...
      surveyBrandCount = 0;
      if (
        surveyBrandSubmissionList[survey_list[survey_list_key].sr_brand_id] &&
        surveyBrandSubmissionList[survey_list[survey_list_key].sr_brand_id]
          .count
      ) {
        surveyBrandCount =
      ...
      survey_list[
        ...
        surveyBrandCount - surveyTaskCount;
      survey_list[
        survey_list_key
      ].dataValues.list_data.entries_after_reward_completed = "NA";
      survey_list[survey_list_key].dataValues.list_data.campaign_budget = "NA";
      survey_list[survey_list_key].dataValues.list_data.brand_data =
        survey_list[survey_list_key].brand;
      survey_list[survey_list_key].dataValues.list_data.tickets = "NA";
      survey_list[survey_list_key].dataValues.list_data.stars_given =
        survey_list[survey_list_key].dataValues.total_stars_given;
      ...
      survey_list[survey_list_key].dataValues.list_data.task_entries =
        surveySubmissionList[taskId] ? surveySubmissionList[taskId].count : 0;
  ...
  if (bonusSetids.length) {
    ...
    for (const bonus_set_key in bonus_set_listing) {
      bonus_set_data[bonus_set_listing[bonus_set_key].bonus_set_id] =
        bonus_set_listing[bonus_set_key];
    }
  }
  if (tasks_list.length) {
    for (const tasks_list_key in tasks_list) {
      tasks_list[tasks_list_key].dataValues.list_data.bonus_set =
        bonus_set_data[tasks_list[tasks_list_key].bonus_set_id]
          ? bonus_set_data[tasks_list[tasks_list_key].bonus_set_id]
          : {};
    }
  }
  if (contest_list.length) {
    for (const contest_list_key in contest_list) {
      contest_list[contest_list_key].dataValues.list_data.bonus_set =
        bonus_set_data[contest_list[contest_list_key].bonus_set_id]
          ? bonus_set_data[contest_list[contest_list_key].bonus_set_id]
          : {};
    }
  }
  ...
  if (tasks_list_data.length) {
    for (const tasks_list_key in tasks_list_data) {
      var ta_type =
        tasks_list_data[tasks_list_key].dataValues.task_data.ta_type;
      var task_id = tasks_list_data[tasks_list_key].dataValues.task_id;
      if (ta_type == 5) {
        tasks_list_data[tasks_list_key].dataValues.task_data.ta_stars_per_user =
          tasks_list_data[
            tasks_list_key
          ].dataValues.task_data.sr_stars_per_user;
      }
      tasks_list_data[tasks_list_key].dataValues.task_list_data =
        taskData[task_id + "_" + ta_type] &&
        taskData[task_id + "_" + ta_type].dataValues.list_data
          ? taskData[task_id + "_" + ta_type].dataValues.list_data
          : {};
    }
  }
...
exports.jsonlisting = async (req, res) => {
  ...
  surveyCompletedTaskIds = surveyCompletedTaskIds.filter(
    (v, i, a) => a.indexOf(v) === i
  );
  // return res.status(200).send({
  //     message : surveyCompletedTaskIds
  // });
  let watchAdsSubmitOptions = {
    attributes: ["watch_ads_task_id"],
    where: {
      u_id: Uid,
  ...
  let completedWatchAdSubmitIds = completedWatchAdSubmits
    .map(function (item) {
      return "" + item.watch_ads_task_id + "";
    })
  ...
  if (tasks_list.length) {
    for (const tasks_list_key in tasks_list) {
      ...
      if (ta_type == 5) {
        tasks_list[tasks_list_key].dataValues.task_data.ta_stars_per_user =
          tasks_list[tasks_list_key].dataValues.task_data.sr_stars_per_user;
      }
    }
  }
  res.status(200).send({
...
exports.taskDetail = async (req, res) => {
  ...
  var reward_given_options = {
    ...
    attributes: [
      ...
      [
        sequelize.fn("sum", sequelize.col("rewards_award_coins")),
        "rewards_award_coins",
      ],
    ],
  };
  const reward_given_listing = await db.rewards_given.findOne(
    reward_given_options
  );
  ...
  if (userTaskPost && userTaskPost.ucpl_created_at != undefined) {
    ...
    if (userTaskPost.task_end_date) {
      ...
      var endDateTimestamp = new Date().setDate(
        userTaskPost.ucpl_created_at.getDate() + 1
      );
  ...
  if (task.bonus_reward_type == "2") {
    ...
    if (task.bonus_set_id) {
      ...
      if (bonusSetDetails) {
        if (bonusSetDetails.bonus_set_start_date) {
          ...
          var bonus_set_end_date = startDate.setDate(
            startDate.getDate() + bonusSetDetails.bonus_set_duration
          );
          bonus_set_end_date.toLocaleString("en-US", {
            timeZone: "Asia/Calcutta",
          });
          let bonus_set_end_date_new = new Date(bonus_set_end_date);
          if (
            bonusSetDetails.bonus_set_start_date.getTime() <= todayDate &&
            todayDate <= bonus_set_end_date_new.getTime()
          ) {
    ...
    if (is_bonus_set_active == 0) {
      ...
      bonus_set_list.forEach((element) => {
        if (element.bonus_set_start_date) {
          ...
          var bonus_set_end_date = startDate.setDate(
            startDate.getDate() + element.bonus_set_duration
          );
          bonus_set_end_date.toLocaleString("en-US", {
            timeZone: "Asia/Calcutta",
          });
          let bonus_set_end_date_new = new Date(bonus_set_end_date);
          if (
            element.bonus_set_start_date.getTime() <= todayDate &&
            todayDate <= bonus_set_end_date_new.getTime() &&
            is_bonus_set_active == 0
          ) {
            is_bonus_set_active = 1;
    ...
    if (
      bonusSetActiveDetails &&
      bonusSetActiveDetails.bonus_set_id != undefined
    ) {
      ...
      task.dataValues.total_bonus_set_tickets = total_tickets_detail
        ? total_tickets_detail.total_tickets
        : 0;
  ...
  try {
    ...
    for (i in req.files) {
      ...
      if (match.indexOf(req.files[i].mimetype) === -1) {
        sharp(req.files[i].path)
          ...
          .withMetadata()
          .toFile(
            "uploads/thumbnails/" + req.files[i].filename,
            (err, resizeImage) => {
              if (err) {
                logger.log(
                  "error",
        ...
        ffmpeg(req.files[i].path)
          ...
          .on("error", function (err) {
            logger.log(
              "error",
              err +
                ":Error creating video thumbnail for " +
                req.body.actionID +
                req.body.tblAlias
            );
          })
    ...
    if (req.body.tblAlias == "post" && media_action == "thumb") {
      ...
      updateData["ucpl_content_data"] = {
        ...
        media: post_details.ucpl_content_data.media
          ? post_details.ucpl_content_data.media
          : [],
        thumbs: x,
      };
        ...
        let updateTrans = {
          trx_source: {
            ...
            post_details: {
              ...
              insta_friends: post_details.ucpl_content_data.insta_friends,
              media: x,
              thumb: post_details.ucpl_content_data.thumb
                ? post_details.ucpl_content_data.thumb
                : [],
            },
            task_details: Transactions_details.trx_source.task_details,
            post_id: post_details.ucpl_id,
    ...
    if (req.body.tblAlias == "task") {
      ...
      if (
        task_details[media_action] !== null &&
        Object.keys(task_details[media_action]).length > 0 &&
        media_action == "ta_post_insp_image"
      ) {
    ...
    if (req.body.tblAlias == "level_task") {
      ...
      levelTask
        ...
        .catch((err) => {
          logger.log(
            "error",
            err + ":Error updating task banner image with id=" + levelTaskID
          );
          res.status(500).send({
      ...
      levelTask
        ...
        .then((num) => {
          if (num == 1) {
            res.status(200).send({
              message:
                "shipping confirmation product image uploaded successfully.",
            });
            return;
          } else {
            res.status(400).send({
        ...
        .catch((err) => {
          logger.log(
            "error",
            err +
              ":Error updating shipping confirmation product image with id=" +
              scId
          );
          res.status(500).send({
    ...
    if (req.body.tblAlias == "user") {
      ...
      User_profile.update(updateVals, {
        ...
        .catch((err) => {
          logger.log(
            "error",
            err + ":Error updating user profile pic with id=" + userID
          );
          res.status(500).send({
...
exports.createNewContest = async (req, res) => {
  ...
  const data = {
    ...
    ct_desc: body.hasOwnProperty("Task description")
      ? req.body["Task description"]
      : "",
    ct_hashtag: body.hasOwnProperty("Task hashtags")
      ? req.body["Task hashtags"]
      : "",
    ct_token_budget: body.hasOwnProperty("Task token budget")
      ? req.body["Task token budget"]
      : 0,
    ct_budget_per_user: body.hasOwnProperty("Task budget per user")
      ? req.body["Task budget per user"]
      : 0,
    ct_stars_per_user: body.hasOwnProperty("Task stars per user")
      ? req.body["Task stars per user"]
      : 0,
    ct_energy_per_user: body.hasOwnProperty("Task energy per user")
      ? req.body["Task energy per user"]
      : 0,
    ct_total_available: body.hasOwnProperty("Task total available")
      ? req.body["Task total available"]
      : 0,
    ct_estimated_user: body.hasOwnProperty("Task estimated user")
      ? req.body["Task estimated user"]
      : 0,
    ct_do: body.hasOwnProperty("Task do") ? req.body["Task do"] : "",
    ct_dont_do: body.hasOwnProperty("Task dont do")
      ? req.body["Task dont do"]
      : "",
    ct_insta_question: body.hasOwnProperty("Task insta question")
      ? req.body["Task insta question"]
      : "",
    ct_photos_required: body.hasOwnProperty("Task photos required")
      ? req.body["Task photos required"]
      : 0,
    ct_videos_required: body.hasOwnProperty("Task videos required")
      ? req.body["Task videos required"]
      : 0,
    ct_mentioned: body.hasOwnProperty("Task mentioned")
      ? req.body["Task mentioned"]
      : "",
    ct_start_date: body.hasOwnProperty("Task start date")
      ? req.body["Task start date"]
      : new Date(),
    ct_end_date: body.hasOwnProperty("Task end date")
      ? req.body["Task end date"]
      : new Date(new Date().setFullYear(new Date().getFullYear() + 2)),
    ct_status: body.hasOwnProperty("Task status") ? req.body["Task status"] : 0,
    ct_oneline_summary: body.hasOwnProperty("Task oneline summary")
      ? req.body["Task oneline summary"]
      : "",
    ct_contiue_spend_budget: body.hasOwnProperty("Continue after budget spend")
      ? req.body["Continue after budget spend"]
      : false,
    ct_start_voting_date: body.hasOwnProperty("voting start date")
      ? req.body["voting start date"]
      : new Date(),
    ct_end_voting_date: body.hasOwnProperty("voting end date")
      ? req.body["voting end date"]
      : new Date(),
    ct_winner_date: body.hasOwnProperty("winner date")
      ? req.body["winner date"]
      : new Date(),
    ct_winner_token: body.hasOwnProperty("winner token")
      ? req.body["winner token"]
      : 0,
  };
  Contest.create(data)
    .then((data) => {
...
exports.listingContest = async (req, res) => {
  ...
  if (
    req.query.sortVal &&
    (req.query.sortBy == "ct_name" || req.query.sortBy == "ct_type")
  ) {
    var sortValue = req.query.sortVal.trim();
    options.where = {
      [sortBy]: {
...
exports.userTasksState = async (req, res) => {
  ...
  var invitation_options = {
    order: [["users_invitation_id", "DESC"]],
    attributes: [
  ...
  const user_invitation_listing = await db.users_invitation.findAll(
    invitation_options
  );
  var bonus_task_data = {};
  var survey_data = {};
  ...
  if (user_invitation_listing.length) {
    for (var user_invitation_key in user_invitation_listing) {
      if (
        user_invitation_listing[user_invitation_key]
          .users_invitation_object_id == 1
      ) {
        // survey
        survey_data[
          user_invitation_listing[
            user_invitation_key
          ].users_invitation_recipient_user_id
        ] =
          user_invitation_listing[
  ...
  var options = {
    attributes: [
      ["sr_hashtags", "task_hashtag"],
      ["sr_enddate_time", "task_endtime"],
      "sr_title",
    ],
    where: {},
  };
  ...
  if (survey_user_state.length) {
    for (var survey_key in survey_user_state) {
      ...
      surveyDetail["task_event_id"] =
        survey_user_state[
          survey_key
        ].dataValues.survey_user_completes[0].dataValues.task_event_id;

      surveyDetail["task_title"] =
        survey_user_state[survey_key].dataValues.sr_title;
      surveyDetail["task_status"] =
        survey_user_state[
          survey_key
        ].dataValues.survey_user_completes[0].dataValues.task_status;
      surveyDetail["user_id"] =
        survey_user_state[
          survey_key
        ].dataValues.survey_user_completes[0].dataValues.user_id;
      ...
      surveyDetail["task_endtime"] =
        survey_user_state[survey_key].dataValues.task_endtime;
  ...
  var bonus_options = {
    attributes: [
      ["bonus_task_id", "task_event_id"],
      ["bonus_task_completion_date", "task_title"],
      ["bonus_task_is_finished", "task_status"],
      ["bonus_task_usr_id", "user_id"],
      ["bonus_task_hashtag", "task_hashtag"],
      ["bonus_task_completion_date", "task_endtime"],
    ],
    where: {
      bonus_task_is_finished: 0,
      bonus_task_usr_id: all_bonus_task_userids,
    },
  };
  ...
  if (bonus_task_result.length) {
    for (var bonus_task_key in bonus_task_result) {
      bonus_task_result[bonus_task_key].dataValues.task_event_type =
        "Bonus Task";
    }
  }
...
exports.taskUsersListing = async (req, res) => {
  ...
  if (contentTypeText == "Task" || contentTypeText == "Contest") {
    ...
    if (reward_given_listing.length) {
      for (const reward_given_key in reward_given_listing) {
        ...
        rewardBrandTokens[reward_listing.rewards_award_user_id].push(
          reward_listing.rewards_award_token
        );
        rewardBrandStars[reward_listing.rewards_award_user_id].push(
          reward_listing.rewards_award_stars
        );
      }
    }
    ...
    const bonus_ticket_listing = await db.bonus_ticket_details.findAll(
      bonus_ticket_options
    );
    if (bonus_ticket_listing.length) {
      for (const bonus_ticket_key in bonus_ticket_listing) {
        var bonus_ticket_list = bonus_ticket_listing[bonus_ticket_key];
        ticketsEarned[bonus_ticket_list.user_id] =
          bonus_ticket_list.tickets_earned;
      }
    }
  }
  ...
  if (userList.length) {
    for (const userListKey in userList) {
      ...
      userList[userListKey].dataValues.tokens_earned = rewardBrandTokens[userId]
        ? rewardBrandTokens[userId].reduce((a, b) => a + b, 0)
        : 0;
      userList[userListKey].dataValues.stars_earned = rewardBrandStars[userId]
        ? rewardBrandStars[userId].reduce((a, b) => a + b, 0)
        : 0;
      userList[userListKey].dataValues.tickets_earned = ticketsEarned[userId]
