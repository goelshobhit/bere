...
exports.createWatchAdsTask = async(req, res) => {
    ...
    const watchAdsTask = {
        ...
        "campaign_type": body.hasOwnProperty("Campaign Type") ? body["Campaign Type"] : 0,
        "budget": body.hasOwnProperty("Budget") ? body["Budget"] : 0,
        "budget_left": body.hasOwnProperty("Budget Left") ? body["Budget Left"] : 0,
        "tokens_given": body.hasOwnProperty("Tokens Given") ? body["Tokens Given"] : 0,
        ...
        logger.log("error", "Some error occurred while creating the Watch Ads Task=" + err);
        res.status(500).send({
            message: err.message || "Some error occurred while creating the Watch Ads Task."
        });
    })
};
...
exports.updatewatchAdsTask = async(req, res) => {
    ...
    watchAdTask.update(req.body, {
        ...
        if (num == 1) {
            audit_log.saveAuditLog(req.header(process.env.UKEY_HEADER || "x-api-key"),'update','videoAds',id,result.dataValues,watchAdTaskDetails);
            res.status(200).send({
 ...
 */
 exports.deletewatchAdsTask = async (req, res) => {
    ...
    if(!videoAdsDetails){
        res.status(500).send({
            message: "Could not delete Watch Ads Task with id=" + req.params.watchAdsTaskId
          });
          return;
    }
    watchAdTask.destroy({
