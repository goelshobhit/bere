...
exports.createWatchAdsTaskSubmit = async(req, res) => {
    ...
    const videoAdsSubmit = {
        ...
        "watch_ads_task_id": body.hasOwnProperty("Watch Ads Task Id") ? body["Watch Ads Task Id"] : 0,
        "submit_watch_timestamp": body.hasOwnProperty("Watch Timestamp") ? body["Watch Timestamp"] : "",
        "submit_watch_completion": body.hasOwnProperty("Watch Completion") ? body["Watch Completion"] : 0,
        "submit_timestamp": body.hasOwnProperty("Submit Timestamp") ? body["Submit Timestamp"] : "",
        "submit_reward_ack": body.hasOwnProperty("Reward Ack") ? body["Reward Ack"] : 0,
        }
    watchAdsTaskSubmit.create(videoAdsSubmit).then(data => {
            audit_log.saveAuditLog(req.header(process.env.UKEY_HEADER || "x-api-key"),'add','todayTimeStamp',data.watch_ads_task_submit_id,data.dataValues);
        res.status(201).send({			
            msg: "Watch Ads Task Submit Created Successfully",
            watchAdsTaskSubmitId: data.watch_ads_task_submit_id
        });
    }).catch(err => {
        logger.log("error", "Some error occurred while creating the Watch Ads Task Submit=" + err);
        res.status(500).send({
            message: err.message || "Some error occurred while creating the Watch Ads Task Submit."
        });
    })
};
...
exports.updatewatchAdsTaskSubmit = async(req, res) => {
    ...
    watchAdsTaskSubmit.update(req.body, {
		returning:true,
        where: {
            watch_ads_task_submit_id: id
        }
        ...
        if (num == 1) {
            audit_log.saveAuditLog(req.header(process.env.UKEY_HEADER || "x-api-key"),'update','videoAdsSubmit',id,result.dataValues,watchAdsTaskSubmitDetails);
            res.status(200).send({
        ...
        logger.log("error", err+":Error updating Watch Ads Task Submit with id=" + id);
        console.log(err)
        res.status(500).send({
 ...
 */
 exports.deletewatchAdsTaskSubmit = async (req, res) => {
    ...
    if(!videoAdsDetails){
        res.status(500).send({
            message: "Could not delete Watch Ads Task Submit with id=" + req.params.watchAdsTaskSubmitId
          });
          return;
    }
    watchAdsTaskSubmit.destroy({
        where: { 
            watch_ads_task_submit_id: req.params.watchAdsTaskSubmitId
        }
      })
