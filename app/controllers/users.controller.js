 ...
 * @return {Promise}
 */
const fetchCmsDetails = async (userID, pageSize, skipCount) => {
  var level_options = {
    include: [
      {
        model: db.level_task,
  ...
  try {
    ...
    userDetails = await Promise.all(
      users_list.map(async (user) => {
        try {
          let user_cms_details = {};
          user_cms_details = await fetchCmsDetails(
            user.u_id,
            pageSize,
            skipCount
          );
          console.log("uid ", user.u_id, " ", typeof user_cms_details);
          if (user_cms_details) {
            return {
              ...user,
              user_cms_details,
