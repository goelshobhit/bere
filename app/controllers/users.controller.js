...
exports.userDetailForAdmin = async (req, res) => {
    ...
    const User = await Users.findOne({
        include: [{
            model: db.user_profile,
            attributes: ['u_display_name', 'u_address', 'u_city', 'u_state', 'u_country', 'u_zipcode', 'u_prof_img_path', 'u_status', 'u_stars', 'u_budget', 'u_energy', 'u_brandscore']
        },
        // {
        //     model: db.user_social_ext
        // },
    ...
    let [taskLevelTwoCount, tasklevelTwoDecline, taskLevelThreeCount, tasklevelThreeDecline] = [[], [], [], []];
    let [tasklevelTwoComplete, tasklevelThreeComplete, tasklevelTwoInComplete, tasklevelThreeInComplete] = [[], [], [], []];
    ...
    if (reward_given_listing.length) {
        ...
        for (const reward_given_key in reward_given_listing) {
            ...
            rewardBrandTokens[reward_listing.rewards_brand_id].push(reward_listing.rewards_award_token);
            rewardBrandStars[reward_listing.rewards_brand_id].push(reward_listing.rewards_award_stars);
            if (reward_listing.reward_center != undefined) {
                ...
                } else if (reward_listing.reward_center.reward_center_reward_type == 1) {
                    present.push(reward_listing.rewards_award_id);
                } else if (reward_listing.reward_center.reward_center_reward_type == 2) {
                    chest.push(reward_listing.rewards_award_id);
                } else if (reward_listing.reward_center.reward_center_reward_type == 3) {
    ...
    if (survey_listing.length) {
        for (const survey_listing_Key in survey_listing) {
            const survey_brand_id = survey_listing[survey_listing_Key].dataValues.survey.sr_brand_id;
    ...
    if (User.user_content_posts.length) {
        ...
        userContentPosts.forEach(element => {
            ...
            if (element.upcl_brand_details) {
                ...
                brandData[element.upcl_brand_details.id] = element.upcl_brand_details.name;
            }
            ...
            if (element.post_comments.length) {
                brandCommentData[element.upcl_brand_details.id].push(element.post_comments);
            }
        ...
        if (brandIds.length) {
            ...
            for (const brandUniqueKey in brandUniqueIds) {
                ...
                brandDetail[brand_name]['User Engagment Rate'] = brandCommentData[brand_id][0] ? brandCommentData[brand_id][0].length : 0;
                brandDetail[brand_name]['last participated date'] = brandContent[brand_id][0]['ucpl_created_at'];
                brandDetail[brand_name]['Brand Shared'] = 0;
                brandDetail[brand_name]['Tokens Earned'] = rewardBrandTokens[brand_id] ? rewardBrandTokens[brand_id].reduce((a, b) => a + b, 0) : 0;
                brandDetail[brand_name]['Stars Earned'] = rewardBrandStars[brand_id] ? rewardBrandStars[brand_id].reduce((a, b) => a + b, 0) : 0;
                ...
                brandDetail[brand_name]['Content Tasks Entered'] = brandContent[brand_id].length;
                ...
                if (brandTaskIds[brand_id].length) {
                    ...
                    brandDetail[brand_name]['Tasks Entered'] = brandTaskIds[brand_id].length;
                }
                if (brandContestIds[brand_id].length) {
                    //var brandContestUniqueIds = brandContestIds[brand_id].filter((v, i, a) => a.indexOf(v) === i);
                    brandDetail[brand_name]['Contest Tasks Entered'] = brandContestIds[brand_id].length;
                }
                brandDetail[brand_name]['Surveys Questions tasks'] = brandSurveyIds[brand_id] ? brandSurveyIds[brand_id].length : 0;
                brandDetail[brand_name]['Ads watched'] = videoAddIds[brand_id] ? videoAddIds[brand_id].length : 0;
                brandDetail[brand_name]['Tickets'] = 0;
                brandDetail[brand_name]['Bonuses Won'] = bonusRewardIds[brand_id] ? bonusRewardIds[brand_id].length : 0;
                brandDetail[brand_name]['bonus tasks incomplete'] = brandBonusTaskIncompleteIds[brand_id] ? brandBonusTaskIncompleteIds[brand_id].length : 0;
                brandDetail[brand_name]['Bonus task completed'] = brandBonusTaskCompleteIds[brand_id] ? brandBonusTaskCompleteIds[brand_id].length : 0;
                brandDetail[brand_name]['offers sent tier 2'] = (taskLevelBrandTwoCount && taskLevelBrandTwoCount[brand_id]) ? taskLevelBrandTwoCount[brand_id].length : 0;
                brandDetail[brand_name]['completed tier 2'] = (tasklevelBrandTwoComplete && tasklevelBrandTwoComplete[brand_id]) ? tasklevelBrandTwoComplete[brand_id].length : 0;
                brandDetail[brand_name]['accepted not completed tier 2'] = (tasklevelBrandTwoInComplete && tasklevelBrandTwoInComplete[brand_id]) ? tasklevelBrandTwoInComplete[brand_id].length : 0;
                brandDetail[brand_name]['not accepted tier 2 declined'] = tasklevelBrandTwoDecline[brand_id] ? tasklevelBrandTwoDecline[brand_id].length : 0;
                brandDetail[brand_name]['completed tier 3'] = tasklevelBrandThreeComplete[brand_id] ? tasklevelBrandThreeComplete[brand_id].length : 0;
                brandDetail[brand_name]['offers sent tier 3'] = taskLevelBrandThreeCount[brand_id] ? taskLevelBrandThreeCount[brand_id].length : 0;
                brandDetail[brand_name]['accepted not completed tier 3'] = tasklevelBrandThreeInComplete[brand_id] ? tasklevelBrandThreeInComplete[brand_id].length : 0;
                brandDetail[brand_name]['not accepted tier 3 declined'] = tasklevelBrandThreeDecline[brand_id] ? tasklevelBrandThreeDecline[brand_id].length : 0;
            }
